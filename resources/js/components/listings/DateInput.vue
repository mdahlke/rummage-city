<template>
    <div class="listing-dates__date">
        ...
        <button class="btn btn-link remove-day"
                ...
                :data-index="this.$vnode.key"
                title="Remove day"
                aria-label="delete"><i class="fad fa-trash"></i>
        </button>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        watch: {
            start() {
                // specify the slot to update (start/end)
                this.$emit('update', {slot: 'start', value: this.start});
            },
            end() {
                // specify the slot to update (start/end)
                this.$emit('update', {slot: 'end', value: this.end});
            }
        },
        mounted() {
            if (this.start_date) {
